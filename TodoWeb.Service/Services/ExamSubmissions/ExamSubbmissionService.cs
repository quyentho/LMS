...
namespace TodoWeb.Service.Services.ExamSubmissions
{
    public class ExamSubbmissionService : IExamSubbmissionService
    {
        ...
        public int CreateStudentExamSubmission(StudentExamSubmissionCreateModel newStudentExamSubmission)
        {
            ...
            var exam = _context.Exams.FirstOrDefault(x => x.Id == newStudentExamSubmission.ExamId && x.Status != Status.Deleted);
            if (exam == null)
            {
                return -1;
            }
            ...
            var questions = _context.Questions.Where(question => questionIds.Contains(question.Id) && question.Status != Status.Deleted);
